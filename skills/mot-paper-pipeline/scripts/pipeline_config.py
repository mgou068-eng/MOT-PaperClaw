#!/usr/bin/env python3
"""Shared runtime configuration for the MOT paper pipeline."""
...
def _env_alias(primary: str, legacy: str, default: str | None = None) -> str | None:
    return _env(primary, _env(legacy, default))
...
def resolve_workspace_root() -> tuple[Path, str | None]:
    configured = _env_alias("MOT_WORKSPACE", "RS_WORKSPACE")
    if not configured:
    ...
    reason = f"MOT_WORKSPACE={configured} is unavailable, fallback to {ROOT_DIR}"
    return ROOT_DIR, reason


def build_runtime_env(extra: dict[str, str] | None = None) -> dict[str, str]:
    env = os.environ.copy()
    proxy_url = _env_alias("MOT_PROXY_URL", "RS_PROXY_URL")
    if proxy_url:
...
def load_config() -> PipelineConfig:
    github_repo = _env_alias("MOT_GITHUB_REPO", "RS_GITHUB_REPO", "your-github-user/MOT-PaperClaw")
    temp_dir = Path(_env_alias("MOT_TEMP_DIR", "RS_TEMP_DIR", "/tmp")).expanduser()
    ...
    return PipelineConfig(
        ...
        arxiv_user_agent=_env("ARXIV_USER_AGENT", f"MOT-PaperClaw/1.0 (+https://github.com/{github_repo})"),
        filter_keywords_path=Path(_env_alias("MOT_FILTER_KEYWORDS_FILE", "RS_FILTER_KEYWORDS_FILE", str(root_dir / "scripts" / "config" / "filter_keywords.json"))).expanduser(),
        filter_prompt_path=Path(_env_alias("MOT_FILTER_PROMPT_FILE", "RS_FILTER_PROMPT_FILE", str(root_dir / "scripts" / "prompts" / "filter_cross_prompt.md"))).expanduser(),
...
def get_repo(config: PipelineConfig | None = None):
    cfg = config or load_config()
    if cfg.github_repo.startswith("your-github-user/"):
        raise RuntimeError("Set MOT_GITHUB_REPO to the target owner/repository before running")
