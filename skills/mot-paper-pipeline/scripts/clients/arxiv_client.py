...
from services.filter_assets import load_mot_query_terms, load_mot_signal_patterns


CONFIG = load_config()
install_urllib_proxy()
MOT_QUERY_TERMS = load_mot_query_terms()
MOT_MATCH_PATTERNS = load_mot_signal_patterns()
ATOM_NAMESPACE = {
...
def has_multi_object_tracking_signal(text: str) -> bool:
    return any(pattern.search(text) for pattern in MOT_MATCH_PATTERNS)
...
def fetch_recent_candidates(
    ...
    query_parts = []
    for keyword in MOT_QUERY_TERMS:
        if " " in keyword:
    ...
    def run_query(query: str, max_scan: int, page_size: int) -> list[dict[str, str]]:
        ...
        for start in range(0, max_scan, page_size):
            ...
            for entry in entries:
                ...
                text = f"{title}\n{abstract}"
                if not has_multi_object_tracking_signal(text):
                    continue
